...
return [
    ...
    // Same local-dev CA bundle workaround already used in SmsService for Vonage —
    // XAMPP on Windows doesn't ship a configured cacert.pem, so outbound cURL/TLS
    // fails locally. Production has never had this problem for outbound HTTP.
    // env('APP_ENV'), not app()->isLocal() — app() isn't safe to call this early
    // in config-file evaluation (breaks bootstrap, confirmed while testing this).
    'http_ssl_verify_peer' => env('APP_ENV') !== 'local',
